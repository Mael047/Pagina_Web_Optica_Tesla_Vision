...
function guardarImagenCarrusel($base64) {
...
$imagen = guardarImagenCarrusel($imagenBase64);
if (!$imagen) {
...
try {
    $conn = getConnection();
    $stmt = $conn->prepare("INSERT INTO carrusel (imagen, orden) VALUES (:imagen, :orden)");
    $stmt->execute(['imagen' => $imagen, 'orden' => $orden]);
    echo json_encode(["status" => "success", "mensaje" => "Imagen agregada al carrusel"]);
