...
try {
    $conn = getConnection();
    $stmt = $conn->prepare("SELECT imagen FROM carrusel WHERE id = :id");
    $stmt->execute(['id' => $id]);
    $row = $stmt->fetch();
    ...
    $stmt = $conn->prepare("DELETE FROM carrusel WHERE id = :id");
    $stmt->execute(['id' => $id]);
    echo json_encode(["status" => "success", "mensaje" => "Imagen eliminada del carrusel"]);
