...
try {
    $conn = getConnection();
    $stmt = $conn->prepare("SELECT imagen FROM testimonios WHERE id = :id");
    $stmt->execute(['id' => $id]);
    $row = $stmt->fetch();
    ...
    $stmt = $conn->prepare("DELETE FROM testimonios WHERE id = :id");
    $stmt->execute(['id' => $id]);
    echo json_encode(["status" => "success", "mensaje" => "Testimonio eliminado"]);
